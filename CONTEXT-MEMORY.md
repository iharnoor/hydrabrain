...
- **`cli.py`** — `hydrabrain` CLI, 15 commands (`status` / `capture` / `ingest` / `sync` / `read` / `search` / `think` / `chat` / `web` / `briefing` / `enrich` / `graph` / `export` / `serve` / `bench`), with global `--tenant` (brain) and `--source` (namespace) flags. Backed by HydraDB via `BrainEngine`.
