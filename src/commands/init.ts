...
async function initPGLite(opts: {
  ...
  // B.1: ALWAYS configure gateway before initSchema (was previously
  // gated on aiOpts.embedding_model || chat_model). Returns resolved
  // values so we can both print them and persist them in B.4.
  const resolved = await configureGatewayWithMergedPrecedence(opts.aiOpts);
  printResolvedAIChoice(resolved, opts.aiOpts);

  const engine = await createEngine({ engine: 'pglite' });
  try {
    await engine.connect({ database_path: dbPath, engine: 'pglite' });

    // B.5: extend the v0.28.5 mismatch guard to fire even when the user
    // didn't pass `--embedding-dimensions` explicitly. After B.1 the
    // gateway always resolves to SOME dim; if that dim differs from an
    // existing column, refuse loudly with the wipe-and-reinit recipe.
    // This catches the bug class where a user with a 1536 brain runs a
    // bare `gbrain init --pglite` after upgrading to v0.36+ and silently
    // ends up with runtime ZE/1280 against a 1536 column.
    {
      ...
      if (existing.exists && existing.dims !== null && existing.dims !== resolved.embedding_dimensions) {
        console.error('\n' + embeddingMismatchMessage({
          currentDims: existing.dims,
          requestedDims: resolved.embedding_dimensions,
          requestedModel: resolved.embedding_model,
          source: 'init',
          engineKind: 'pglite',
          databasePath: dbPath,
        }) + '\n');
        if (opts.jsonOutput) {
          console.log(JSON.stringify({
            ...
            requested_dims: resolved.embedding_dimensions,
          }));
        }
        process.exit(1);
      }
    ...
    // B.4: merge existing file-plane config + CLI flags + resolved defaults.
    // Precedence: CLI > existing file > resolved defaults. Env vars are NOT
    // persisted (they stay an env-only runtime escape hatch). Use
    // loadConfigFileOnly() — loadConfig() would poison config.json with
    ...
    // Backstop: persist the resolved defaults so `gbrain config show`
    // displays them. Otherwise a bare install would show no
    // embedding_model / dimensions in config.json and users would assume
    // they're unset.
    if (!merged.embedding_model) merged.embedding_model = resolved.embedding_model;
    if (!merged.embedding_dimensions) merged.embedding_dimensions = resolved.embedding_dimensions;
    saveConfig(merged);
...
async function initPostgres(opts: {
  ...
  // B.2: ALWAYS configure gateway (was previously gated on flags).
  const resolved = await configureGatewayWithMergedPrecedence(opts.aiOpts);
  printResolvedAIChoice(resolved, opts.aiOpts);
  ...
  try {
    ...
    // B.5: extend the v0.28.5 mismatch guard (Postgres path mirrors PGLite).
    {
      const { readContentChunksEmbeddingDim, embeddingMismatchMessage } = await import('../core/embedding-dim-check.ts');
      const existing = await readContentChunksEmbeddingDim(engine);
      if (existing.exists && existing.dims !== null && existing.dims !== resolved.embedding_dimensions) {
    ...
    // B.4 (Postgres mirror): merge existing file-plane + CLI flags +
    // resolved defaults. Same precedence as PGLite path.
    const existingFile = loadConfigFileOnly() ?? ({} as GBrainConfig);
    const config: GBrainConfig = {
      ...existingFile,
      engine: 'postgres',
    ...
    if (!config.embedding_model) config.embedding_model = resolved.embedding_model;
    if (!config.embedding_dimensions) config.embedding_dimensions = resolved.embedding_dimensions;
    saveConfig(config);
